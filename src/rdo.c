...
static const int    COEFF_COST_QP_LIMIT  = 27;
...
uint32_t kvz_get_coeff_cost(const encoder_state_t * const state,
                            ...
                            int8_t scan_mode)
{
  if (state->encoder_control->cfg.rdo > 0 || state->qp > COEFF_COST_QP_LIMIT) {
    // Use CABAC for coeff coding cost when RDO level is greater than zero
    // or QP is high. At high QPs, there are less coefficients so CABAC is
    // almost as fast as the fast cost estimation.
